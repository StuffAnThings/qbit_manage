...
def test_get_logs_reads_rotated_log(log_api):
    (log_api.logs_path / "qbit_manage.1.log").write_text("rotated\n", encoding="utf-8")

    assert get_logs(log_api, log_filename="qbit_manage.1.log") == {"logs": ["rotated"]}
...
def test_list_log_files_includes_rotations_in_natural_order(log_api):
    ...
    assert result == {"log_files": ["activity.log", "activity.2.log", "activity.10.log", "other.1.log"]}
