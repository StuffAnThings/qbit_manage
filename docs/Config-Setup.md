...
| `rem_unregistered_confirm_minutes`  | Require a torrent to stay unregistered for this many minutes before it is removed. The first time it is seen unregistered it is tagged with `unregistered_tag`; it is only removed on a later run once this much time has elapsed and it is still unregistered. A working (or no-longer-unregistered) tracker clears the flag. Set above your tracker announce interval to protect against a tracker that transiently reports "unregistered"/"not found" during an outage. Set to 0 to disable (remove immediately). Note: dry-run never writes the flag, so it cannot simulate the second-pass removal. | 0 | <center>❌</center> |
| `unregistered_tag`                  | Tag used to mark a torrent pending removal while `rem_unregistered_confirm_minutes` is enabled (a timestamp is appended to it).                                                                                                                                                                                                                  | unregisteredCheck         | <center>❌</center> |
